...
export const loginUser = async (phone: string, password: string): Promise<User> => {
  ...
  if (useMockMode) {
    ...
    if (!user) {
      // Si se intenta con el usuario "Hugol", crearlo automáticamente
      if (phone === '0987654321') {
        const mockUser: User = {
          id: `user-${Date.now()}`,
          name: 'Hugol',
          phone,
          token: generateMockToken(),
          quizCompleted: true // El usuario Hugol siempre tiene el quiz completado
        };
        mockUsers[phone] = mockUser;
        console.log('👤 Usuario de prueba Hugol creado:', mockUser);
...
export const updateQuizStatus = async (completed: boolean): Promise<boolean> => {
  ...
  if (useMockMode) {
    ...
    // Si el estado es completado, actualizamos localStorage inmediatamente
    // ya que esto es crítico para la experiencia del usuario
    if (completed) {
      localStorage.setItem('quizCompleted', 'true');
      console.log('Estado del quiz actualizado en localStorage: true');
    }
    ...
    if (phone && mockUsers[phone]) {
      ...
      // Incluso si no encontramos el usuario, guardar el estado en localStorage
      // Esto asegura que el estado del quiz se mantenga entre sesiones
      if (completed) {
        localStorage.setItem('quizCompleted', 'true');
        console.log('Estado del quiz actualizado solo en localStorage: true');
        return true;
      }
  ...
  // Parte para backend real (no modificada)
  try {
    ...
    // Si llegamos aquí, actualizamos localStorage también para mayor seguridad
    if (completed) {
      localStorage.setItem('quizCompleted', 'true');
    }
    ...
    // Si hay error pero estamos intentando marcar como completado,
    // actualizamos localStorage de todas formas
    if (completed) {
      localStorage.setItem('quizCompleted', 'true');
      return true;
    }
